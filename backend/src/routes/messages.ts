...
async function auth(req:any,res:any,next:any){ const h=req.headers.authorization; if(!h) return res.status(401).json({ error:'unauth' }); try{ const token = h.split(' ')[1]; const decoded:any = jwt.verify(token, process.env.JWT_SECRET || 'devsecret'); const userId = decoded.id; (req as any).userId = userId; next(); }catch(e){ return res.status(401).json({ error:'invalid' }); } }
