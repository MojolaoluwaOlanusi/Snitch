...
const PostPage = () => {
    ...
    useEffect(() => {
        if (singlePost?.author) {
            ...
            if (typeof singlePost.author === 'string') {
                ...
                console.log("Author is already an object");
    ...
    const reportFunction = (e) => {
        reportPost({id: singlePost?._id, reason: {reason: e.target.value}});
    ...
    const isPostOwner = typeof singlePost?.author === 'string' ? singlePost.author === authUserId : singlePost?.author?._id === authUserId;
            ...
            <div className='flex-1 flex flex-col bg-gradient-to-br from-blue-50 to-white overflow-auto'>
                ...
                <div className="w-full p-4">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        <Link to={`/profile/${authorData?.username || '#'}`} className="flex-shrink-0">
                            <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-blue-200">
                                <img 
                                    src={authorData?.avatarUrl || "/avatar-placeholder.png"} 
                        ...
                        <div className="flex-1">
                            <Link to={`/profile/${authorData?.username || '#'}`} className="font-bold text-gray-900 hover:underline">
                                {authorData?.displayName || "Unknown User"}
                            </Link>
                            <p className="text-gray-500 text-sm">
                                @{authorData?.username || "unknown"} · {formatPostDate(singlePost?.createdAt)}
                            </p>
                        </div>
                        <div className="relative">
                            <button
                                className="p-2 hover:bg-gray-100 rounded-full transition-colors"
                                onClick={() => setReportSelectVisible(!reportSelectVisible)}
                            >
                                {(editingPostId === singlePost?._id) && <LoadingSpinner size='sm'/>}
                                {(deletingPostId === singlePost?._id) && <LoadingSpinner size='sm'/>}
                                {(reportingPostId === singlePost?._id) && <LoadingSpinner size='sm'/>}
                                {!(editingPostId === singlePost?._id) && !(deletingPostId === singlePost?._id) && !(reportingPostId === singlePost?._id) && <MoreHorizontal className="h-5 w-5 text-gray-700"/>}
                            </button>
                            ...
                            {reportSelectVisible && (
                                <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-lg border border-gray-200 z-50">
                                    <div className="p-2">
                                        ...
                                        {!isPostOwner && (
                                            <button
                                                onClick={() => setReportSelectVisible(true)}
                                        ...
                                        {reportSelectVisible && !isPostOwner && (
                                            <div className="px-3 py-2">
                                                ...
                                                >
                                                    ...
                                                    <option value="cyberbully">Cyberbully</option>
                    ...
                    {/* Post Text */}
                    <div className="mb-4">
                        <p className="text-gray-900 text-lg leading-relaxed whitespace-pre-wrap">{singlePost?.text}</p>
                    ...
                    {/* Reactions Display */}
                    {singlePost?.reaction && singlePost.reaction.length > 0 && (
                        <div className="mb-4">
                            <ReactionsDisplay reactions={singlePost.reaction} />
                        </div>
                    )}
                    ...
                    <div className="flex items-center justify-between py-3 border-y border-gray-200 mb-4">
                        <div className="flex items-center justify-between w-full gap-2">
                            ...
                            <button className="flex items-center justify-center gap-2 text-gray-500 hover:text-sky-500 transition-colors group flex-1 py-2 hover:bg-gray-100 rounded-lg">
                                ...
                                <span className="text-sm">{singlePost?.comments?.length || 0}</span>
                            </button>
                            ...
                            >
                                {isReposting && actionPostId === singlePost._id && <LoadingSpinner size='sm' />}
                                {!isReposting && <BiRepost className='w-5 h-5 group-hover:text-green-500' />}
                                <span className="text-sm">{singlePost?.repostCount || 0}</span>
                            </button>

                            {/* Like */}
                            <button 
                                className={`flex items-center justify-center gap-2 transition-colors group flex-1 py-2 hover:bg-gray-100 rounded-lg ${isLikedByMe ? 'text-pink-500' : 'text-gray-500 hover:text-pink-500'}`}
                                onClick={() => {
                                    setActionPostId(singlePost._id);
                                    likePost(singlePost._id);
                                }}
                            >
                                {isLiking && actionPostId === singlePost._id && <LoadingSpinner size='sm' />}
                                {!isLiking && <FaRegHeart className={`w-5 h-5 ${isLikedByMe ? 'text-pink-500' : 'group-hover:text-pink-500'}`} />}
                                <span className="text-sm">{singlePost?.likes?.length || 0}</span>
                            </button>
                            ...
                            <div className="relative flex-1">
                                <button 
                                    ...
                                    onClick={() => setEmojiPickerOpen(emojiPickerOpen === singlePost._id ? null : singlePost._id)}
                                >
                                    {isReacting && actionPostId === singlePost._id && <LoadingSpinner size='sm' />}
                                    {!isReacting && <MdAddReaction className='w-5 h-5 group-hover:text-yellow-500' />}
                                    <span className="text-sm">{reactionCount}</span>
                                </button>
                                <ReactionEmojiPicker
                                    postId={singlePost._id}
                                    isOpen={emojiPickerOpen === singlePost._id}
                                    onClose={() => setEmojiPickerOpen(null)}
                                    onReact={(emoji) => {
                                        setActionPostId(singlePost._id);
                                        reactToPost({id: singlePost._id, reaction: emoji});
                                    }}
                                />
                            </div>
                        </div>
                    ...
                    <div className="border-t border-gray-200 pt-4">
                        ...
                        <form className="flex gap-3 mb-6" onSubmit={handlePostComment}>
                            <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-blue-200 flex-shrink-0">
                                <img 
                                    src={authUser?.avatarUrl || "/avatar-placeholder.png"} 
                                    alt="Your avatar"
                                    className="w-full h-full object-cover"
                                />
                            </div>
                            <div className="flex-1">
                                <textarea
                                    ...
                                    onChange={(e) => setCommentData({ ...commentData, text: e.target.value, postId: singlePost._id })}
                                />
                                <button 
                                    type="submit"
                                    ...
                                    disabled={isCommenting || !commentData.text.trim()}
                                >
                        ...
                        <div className="space-y-4">
                            ...
                            {singlePost?.comments?.map((comment) => (
                                <div key={comment._id} className="flex gap-3">
                                    <Link to={`/profile/${comment?.userUsername}`} className="flex-shrink-0">
                                    ...
                                    <div className="flex-1">
                                        <div className="bg-gray-100 rounded-2xl px-4 py-2">
                                            <div className="flex items-center gap-2 mb-1">
                                                <Link to={`/profile/${comment?.userUsername}`} className="font-bold text-sm hover:underline">
                                                    {comment?.userDisplayName}
                                                </Link>
                                                <span className="text-gray-500 text-xs">@{comment?.userUsername}</span>
                                            </div>
