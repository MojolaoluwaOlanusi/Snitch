...
export default function Login() {
    ...
    return (_jsx("div", { className: "flex items-center justify-center min-h-screen bg-base-200 px-4", children: _jsxs("form", { onSubmit: handleSubmit, className: "bg-base-100 p-8 rounded-xl shadow-lg w-full max-w-sm space-y-4", children: [_jsx("h1", { className: "text-2xl text-base-content font-bold text-center", children: "Admin Login" }), _jsx("input", { type: "email", placeholder: "Email", className: "input input-bordered text-base-content w-full", value: email, "aria-label": "Email", title: "Email", onChange: (e) => setEmail(e.target.value), required: true }), _jsx("input", { type: "password", placeholder: "Password", className: "input input-bordered text-base-content w-full", value: password, "aria-label": "Password", title: "Password", onChange: (e) => setPassword(e.target.value), required: true }), _jsx("button", { className: "btn btn-primary w-full", disabled: loading, "aria-label": "Login", title: "Login", children: loading ? _jsx("span", { className: "loading loading-spinner" }) : 'Login' })] }) }));
}
